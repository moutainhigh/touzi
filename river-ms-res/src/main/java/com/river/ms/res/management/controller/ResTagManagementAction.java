 ...
 */
@RestController
@RequestMapping("/resTagManagement")
public class ResTagManagementAction {
	@Autowired
	MPResTagService service;

	@RequestMapping(value = "/insertOrUpdate", method = RequestMethod.POST)
	public JsonResult insertOrUpdate(HttpServletRequest request, @Valid ResTag entity, BindingResult res) {
		if (res.hasErrors()) {
	...
	public JsonResult select(HttpServletRequest request) {
		EntityWrapper<ResTag> wrapper = new EntityWrapper<>();
		List<ResTag> selectList = this.service.selectList(wrapper);
