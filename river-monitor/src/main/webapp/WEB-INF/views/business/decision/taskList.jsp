...
<head>
	...
	<script src="${ctx }/js/module/business/decision/todo.js" type="text/javascript"></script>
</head>
...
<body>
	...
	<table width="100%" class="need-border">
		...
		<tbody>
			<c:forEach items="${page.result }" var="decision">
				<c:set var="task" value="${decision.task }" />
				<c:set var="pi" value="${decision.processInstance }" />
				<tr id="${decision.id }" tid="${task.id }">
					<td>${decision.projectType }</td>
					<td>${decision.projectSize }</td>
					<td>${decision.userId }</td>
					<td>${decision.applyTime }</td>
					<td>${decision.startTime }</td>
					<td>
					...
					<td>${pi.suspended ? "已挂起" : "正常" }；<b title='流程版本号'>V: ${decision.processDefinition.version }</b></td>
					<td>
						<c:if test="${empty task.assignee }">
							<a class="claim" href="${ctx }/business/decision/task/claim/${task.id}">签收</a>
						</c:if>
