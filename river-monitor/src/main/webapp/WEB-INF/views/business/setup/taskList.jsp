...
<head>
	...
	<script src="${ctx }/js/module/business/setup/todo.js" type="text/javascript"></script>
</head>
...
<body>
	...
	<table width="100%" class="need-border">
		...
		<tbody>
			<c:forEach items="${page.result }" var="setup">
				<c:set var="task" value="${setup.task }" />
				<c:set var="pi" value="${setup.processInstance }" />
				<tr id="${setup.id }" tid="${task.id }">
					<td>${setup.projectType }</td>
					<td>${setup.projectSize }</td>
					<td>${setup.userId }</td>
					<td>${setup.applyTime }</td>
					...
					<td>${pi.suspended ? "已挂起" : "正常" }；<b title='流程版本号'>V: ${setup.processDefinition.version }</b></td>
					<td>
						<c:if test="${empty task.assignee }">
							<a class="claim" href="${ctx }/business/setup/task/claim/${task.id}">签收</a>
						</c:if>
