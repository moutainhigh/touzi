...
<head>
	...
	<script src="${ctx }/js/module/business/feasibility/todo.js" type="text/javascript"></script>
</head>
...
<body>
	...
	<table width="100%" class="need-border">
		...
		<tbody>
			<c:forEach items="${page.result }" var="feasibility">
				<c:set var="task" value="${feasibility.task }" />
				<c:set var="pi" value="${feasibility.processInstance }" />
				<tr id="${feasibility.id }" tid="${task.id }">
					<td>${feasibility.projectType }</td>
					<td>${feasibility.projectSize }</td>
					<td>${feasibility.userId }</td>
					<td>${feasibility.applyTime }</td>
					<td>${feasibility.startTime }</td>
					<td>
					...
					<td>${pi.suspended ? "已挂起" : "正常" }；<b title='流程版本号'>V: ${feasibility.processDefinition.version }</b></td>
					<td>
						<c:if test="${empty task.assignee }">
							<a class="claim" href="${ctx }/business/feasibility/task/claim/${task.id}">签收</a>
						</c:if>
