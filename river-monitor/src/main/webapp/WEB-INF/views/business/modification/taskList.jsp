...
<head>
	...
	<script src="${ctx }/js/module/business/modification/todo.js" type="text/javascript"></script>
</head>
...
<body>
	...
	<table width="100%" class="need-border">
		...
		<tbody>
			<c:forEach items="${page.result }" var="modification">
				<c:set var="task" value="${modification.task }" />
				<c:set var="pi" value="${modification.processInstance }" />
				<tr id="${modification.id }" tid="${task.id }">
					<td>${modification.projectType }</td>
					<td>${modification.projectSize }</td>
					<td>${modification.userId }</td>
					<td>${modification.applyTime }</td>
					<td>${modification.startTime }</td>
					<td>
					...
					<td>${pi.suspended ? "已挂起" : "正常" }；<b title='流程版本号'>V: ${modification.processDefinition.version }</b></td>
					<td>
						<c:if test="${empty task.assignee }">
							<a class="claim" href="${ctx }/business/modification/task/claim/${task.id}">签收</a>
						</c:if>
	...
	<!-- 部门领导审批 -->
	<div id="brainAudit" style="display: none">
