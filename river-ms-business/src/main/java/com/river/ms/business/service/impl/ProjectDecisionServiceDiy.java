...
import com.river.ms.business.mapper.ProjectDecisionDao;
 ...
 */
@Service
public class ProjectDecisionServiceDiy extends ServiceImpl<ProjectDecisionDao, ProjectDecision>
		implements MPProjectDecisionService {
	...
	@Override
	@Transactional
	public Boolean insertProjectDecision(ProjectDecision entity, Map<String, Object> headerMap, String str,
			String docStr, UserEntity user, String partner, String partnerMember) {
		boolean insert = this.insert(entity);
		if (insert) {
			Integer type = EnumFlowType.DECISION.getFlowType();
			...
			if (projectFlowStart != null && projectFlowStart.size() > 0) {
				...
				if (startActiviti.getStatus() == 0) {
					...
					this.projectTodoService.deleteTodo(entity.getProjectId(), null, null, null, 1);
					...
					if (partner != null && !partner.equals("")) {
						ProjectPartner projectPartner = JSONObject.parseObject(partner, ProjectPartner.class);
						this.projectPartnerService.insertProjectPartner(projectPartner, partnerMember, user);
					}
					if (setProjectState) {
						return true;
					} else {
	...
	@Override
	@Transactional
	public Boolean setProjectDecisionState(Long entityId, Integer state, Map<String, Object> headerMap) {
		ProjectDecision entity = this.baseMapper.selectById(entityId);
		if (entity != null) {
	...
	@Override
	@Transactional
	public Boolean updateProjectDecision(HttpServletRequest request, ProjectDecision entity, String docStr) {
