...
import com.river.ms.business.mapper.ProjectModificationDao;
 ...
 */
@Service
public class ProjectModificationServiceDiy extends ServiceImpl<ProjectModificationDao, ProjectModification>
		implements MPProjectModificationService {
	...
	@Override
	@Transactional
	public Boolean insertProjectModification(ProjectModification project, Map<String, Object> headerMap, String str,String docStr,UserEntity user) {

		boolean insert = this.insert(project);
		if (insert) {
			Integer type = EnumFlowType.MODIFICATION.getFlowType();
			...
			if (projectFlowStart != null && projectFlowStart.size() > 0) {
				...
				if (startActiviti.getStatus() == 0) {
					...
					toProject.setProjectState(headerMap, project.getProjectId(), Integer.valueOf(startActiviti.getTaskState()));
					...
					this.projectTodoService.deleteTodo(project.getProjectId(), null, null, null, 0);
					this.projectTodoService.insertTodos(project.getProjectId(), taskCode);
					...
					for (Entry<Long, List<Long>> e : entrySet) {
						if (e.getValue() != null && e.getValue().size() > 0) {
							for (Long l : e.getValue()) {
								this.projectTodoService.insertTodo(project.getProjectId(), e.getKey(), l, null, 0);
							}
						} else {
							this.projectTodoService.insertTodo(project.getProjectId(), e.getKey(), null, null, 0);
						}
					}*/
	...
	@Override
	@Transactional
	public Boolean setProjectModificationState(Long projectId, Integer state, Map<String, Object> headerMap) {
		ProjectModification selectById = this.selectById(projectId);
		if (selectById != null) {
	...
	@Override
	@Transactional
	public Boolean updateProjectModification(HttpServletRequest request, ProjectModification entity,String docStr) {
		...
		if (nodeEnd.getStatus() == 0) {
			if (nodeEnd.getTasks() == null) {
				...
				// 设置项目阶段
				toProject.setProjectStage(headerMap, projectId, projectStageIdByCode.get(new Long(1)),nodeEnd.getTaskMemo());
				toProject.setProjectState(headerMap, projectId, Integer.valueOf(nodeEnd.getTaskState()));
				// 添加操作用户
	...
	@Override
	public List<ProjectModification> getProjectModificationByProjectId(Long projectId,String processInstanceId) {
		List<ProjectModification> projectModificationByProjectId = this.baseMapper
				.getProjectModificationByProjectId(projectId,processInstanceId);
		return projectModificationByProjectId;
	}
